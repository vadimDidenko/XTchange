...
public interface Bitmex {
  ...
  Call<List<BitmexWalletTransaction>> getWalletHistory(
      @Query("count") String count, @Query("start") String start, @Query("reverse") String reverse);
  ...
  @GET("api/v1/trade")
  Call<BitmexPublicTrade[]> getTrades(
      @Query("symbol") String currencyPair, @Query("reverse") Boolean reverse);

  @GET("api/v1/orderBook/L2")
  Call<BitmexPublicOrder[]> getDepth(
      @Query("symbol") String currencyPair, @Query("depth") Double depth);
  ...
  Call<List<BitmexTicker>> getTickers(
  ...
  @GET("api/v1/order")
  Call<List<BitmexPrivateOrder>> getOrders(
