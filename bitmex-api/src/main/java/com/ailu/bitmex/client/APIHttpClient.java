...
public class APIHttpClient {
  ...
  public OkHttpClient client() {
    ...
    if (this.config.isEnableProxy()) {
      clientBuilder.proxy(
          new java.net.Proxy(
              Proxy.Type.HTTP,
              new InetSocketAddress(this.config.getProxyUrl(), this.config.getProxyPort())));
    }
  ...
  private String getSignature(
      String secretKey, String verb, String signPath, String nonce, String body) {
  ...
  private String requestPath(final Request request) {
    ...
    //        if
    // (APIConstants.get.equalsIgnoreCase(method(request))&&requestPath.indexOf("?")!=-1&&requestPath.indexOf("%")==-1&&requestPath.indexOf("&")==-1){
