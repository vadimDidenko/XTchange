...
/**
 * okex.com api doc url: https://github.com/okcoin-okex/API-docs-OKEx.com <code>
 * okexApi.ws()
...
public class OKEx {
  ...
  public interface Api {
    ...
    Call<String> trades(@Query("symbol") String symbol, @Query("contract_type") String type);
    ...
    default WSListener ws() {
      return new WSListener().url(WS_URL).ping("{'event':'ping'}", 30).parse(OKEx::parse);
    }
  ...
  private static final Pattern PATTERN =
      Pattern.compile(
          "ok_sub_(spot|futureusd)_(.+)_(ticker|depth|deals|trades|kline)_?(this_week|next_week|quarter)?_?(1min|1day)?");
  ...
  public static void main(String[] args) {
    try {
      ...
      okexApi
          .ws()
          .send(
              "{'event':'addChannel','channel':'ok_sub_futureusd_btc_kline_this_week_1min'}") // send可以重复调用，也可以传list
          .message(info -> System.out.println("info:" + info)) // 获取解析后的message信息
          .event(
              "message",
              (webSocket, s) -> System.out.println("raw message:" + s)) // 可以通过message事件获取原始字符串
          .start();
    } catch (Exception e) {
      e.printStackTrace();
    }
