...
/**
 * zb.com api doc url: https://www.zb.com/i/developer
 *
 * <pre>
...
public class Zb {
  ...
  public interface Api {
     ...
     * @return asks : 卖方深度, bids : 买方深度, timestamp : 此次深度的产生时间戳
     */
    @GET("depth")
    Call<String> depth(
     ...
     * @return date : 交易时间(时间戳), price : 交易价格, amount : 交易数量, tid : 交易生成ID, type :
     *     交易类型，buy(买)/sell(卖), trade_type : 委托类型，ask(卖)/bid(买)
     */
    @GET("trades")
    Call<String> trades(@Query("market") String tradePair, @Query("since") String since);
     ...
     * @return data : K线内容 moneyType : 买入货币 symbol : 卖出货币 data : 内容说明 [ 1417536000000, 时间戳 2370.16,
     *     开 2380, 高 2352, 低 2367.37, 收 17259.83 交易量 ]
     */
    @GET("kline")
    Call<String> kline(
        @Query("market") String tradePair,
    ...
    default WSListener ws() {
      return new WSListener().url(WS_URL).parse(Zb::parse);
    }
  }
