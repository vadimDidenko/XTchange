...
public class PointExt {
  ...
  private static final Map<Class<?>, PointExt> cache = new HashMap<Class<?>, PointExt>(10);
  ...
  private void concatenatedTags(StringBuilder sb, Object object) throws IllegalAccessException {
    for (Field tag : tags) {
      ...
      sb.append(',').append(tag.getName()).append('=').append(keyEscape(value.toString()));
    }
    sb.append(' ');
  }
  ...
  private void formatedTime(final StringBuilder sb, final Object object)
      throws IllegalAccessException {
    if (this.time == null || this.precision == null) {
      return;
    }
    sb.append(' ')
        .append(TimeUnit.NANOSECONDS.convert(Long.valueOf(time.get(object).toString()), precision));
  }
