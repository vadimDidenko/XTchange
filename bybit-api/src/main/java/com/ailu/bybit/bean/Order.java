...
/**
 * NOTE: OneToken 订单列表 { 'order_id': 'string', UUID类型订单唯一ID 'user_id': 0, 用户ID 'symbol': 'string',
 * 产品类型 'side': 'string', 购买方向 'order_type': 'string', 订单类型 'price': 0, 委托价格 'qty': 0, 委托数量
 * 'time_in_force': 'string', 执行策略 'order_status': 'string', 委托状态:
 * Created:创建订单;Rejected:订单被拒绝;New:订单待成交;PartiallyFilled:订单部分成交;Filled:订单全部成交,Cancelled:订单被取消
 * 'last_exec_time': 0.000000, 最近一次成交时间 'last_exec_price': 0, 最近一次成交价格 'leaves_qty': 0, 剩余委托数量
 * 'cum_exec_qty': 0, 累计成交数量 'cum_exec_value': 0, 累计成交的名义价值 'cum_exec_fee': 0, 累计已成交手续费
 * 'reject_reason': 'string', 被拒单的原因 'order_link_id': 'string',机构自定义订单ID
 * 'created_at':'2018-10-15T04:12:19.000Z', 'updated_at':'2018-10-15T04:12:19.000Z', } @Version
 * 1.0 @Since JDK1.8 @Author mr.wang @Company Henan ailu @Date 2018/12/13 17:31
 */
@Data
public class Order {

  /**
   * order_id : string user_id : 0 symbol : string side : string order_type : string price : 0 qty :
   * 0 time_in_force : string order_status : string last_exec_time : 0 last_exec_price : 0
   * leaves_qty : 0 cum_exec_qty : 0 cum_exec_value : 0 cum_exec_fee : 0 reject_reason : string
   * order_link_id : string created_at : 2018-10-15T04:12:19.000Z updated_at :
   * 2018-10-15T04:12:19.000Z
   */
