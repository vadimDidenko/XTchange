...
public abstract class RippleAdapters {
   ...
   *
   * <p>Counterparties are not mapped since the application calling this should know and keep track
   * of the counterparties it is using in the polling thread.
   */
  public static OrderBook adaptOrderBook(
    ...
    if (counterCounterparty.equals(params.getCounterCounterparty()) == false) {
      throw new IllegalStateException(
          String.format(
              "counter counterparty in Ripple order book %s does not match requested counterparty"
                  + " %s",
   ...
   *
   * <p>Counterparties set in additional data since there is no other way of the application
  ...
  public static UserTrade adaptTrade(
    ...
    if (balanceChanges.size() != 2) {
      logger.warn(
          "for hash[{}] of type[{}] balance changes section should contains 2 currency amounts but"
              + " found {}",
          trade.getHash(),
    ...
    if (preferredBase.contains(Currency.getInstance(balanceChanges.get(0).getCurrency()))) {
      ...
      if (pair.base.getCurrencyCode().equals(balanceChanges.get(0).getCurrency())
        ...
        throw new IllegalStateException(
            String.format(
                "trade history param currency filter specified %s but trade query returned %s and"
                    + " %s",
