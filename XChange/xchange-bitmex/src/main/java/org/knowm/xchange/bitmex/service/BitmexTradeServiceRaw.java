...
public class BitmexTradeServiceRaw extends BitmexBaseService {
  ...
  public List<BitmexPosition> getBitmexPositions(String symbol, String filter) throws IOException {
    ...
    try {
      return bitmex.getPositions(
          apiKey, exchange.getNonceFactory(), signatureCreator, symbol, filter);
    } catch (BitmexException e) {
      throw handleError(e);
    }
  }
  ...
  public List<BitmexPrivateOrder> getBitmexOrders() throws IOException {
    ...
    for (int i = 0; orders.size() % 500 == 0; i++) {
      ...
      orders.addAll(
          orderResponse.stream()
              .filter(bitmexPrivateOrder -> bitmexPrivateOrder.getPrice() != null)
  ...
  public List<BitmexPrivateOrder> getBitmexOrders(
      String symbol, String filter, Date startTime, Date endTime, Integer pageNum, Integer pageSize)
      throws IOException {
    ArrayList<BitmexPrivateOrder> orders = new ArrayList<>();
