...
public interface BinanceApiWebSocketClient extends Closeable {
  ...
  /**
   * Open a new web socket to receive {@link UserDataUpdateEvent userDataUpdateEvents} on a
   * callback.
   *
   ...
   * @return a {@link Closeable} that allows the underlying web socket to be closed.
   */
  Closeable onUserDataUpdateEvent(
      String listenKey, BinanceApiCallback<UserDataUpdateEvent> callback);

  /**
   * Open a new web socket to receive {@link AllMarketTickersEvent allMarketTickersEvents} on a
   * callback.
   *
   * @param callback the callback to call on new events
   * @return a {@link Closeable} that allows the underlying web socket to be closed.
