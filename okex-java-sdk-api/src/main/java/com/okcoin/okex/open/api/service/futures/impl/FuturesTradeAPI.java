...
interface FuturesTradeAPI {
  ...
  Call<JSONObject> cancelOrder(
      @Path("instrument_id") String instrumentId, @Path("order_id") String orderId);

  @POST("/api/futures/v3/cancel_batch_orders/{instrument_id}")
  Call<JSONObject> cancelOrders(
      @Path("instrument_id") String instrumentId, @Body JSONObject order_ids);

  @GET("/api/futures/v3/orders/{instrument_id}")
  Call<JSONObject> getOrders(
      @Path("instrument_id") String instrumentId,
  ...
  Call<JSONObject> getOrder(
      @Path("instrument_id") String instrumentId, @Path("order_id") String orderId);

  @GET("/api/futures/v3/fills")
  Call<JSONArray> getFills(
      @Query("instrument_id") String instrumentId,
      @Query("order_id") String orderId,
      @Query("from") int before,
      @Query("to") int after,
      @Query("limit") int limit);
  ...
  Call<JSONObject> changeLevelRate(
  ...
  @POST("/api/futures/v3/accounts/{currency}/leverage")
  Call<JSONObject> changequancanLevelRate(
      @Path("currency") String currency, @Query("leverage") int leverage);
}
