...
public class SpotProductAPIServiceImpl implements SpotProductAPIService {
  ...
  @Override
  public Book bookProductsByProductId(
      final String product, final String size, final BigDecimal depth) {
    return this.client.executeSync(
        this.spotProductAPI.bookProductsByProductId(product, size, depth));
  }
  ...
  public List<Trade> getTrades(
      final String product, final String from, final String to, final String limit) {
    return this.client.executeSync(this.spotProductAPI.getTrades(product, from, to, limit));
  }

  @Override
  public List<KlineDto> getCandles(
      final String product, final String granularity, final String start, final String end) {
    return this.client.executeSync(
        this.spotProductAPI.getCandles(product, granularity, start, end));
  }

  @Override
  public List<String[]> getCandles_1(
      final String product, final String granularity, final String start, final String end) {
    return this.client.executeSync(
        this.spotProductAPI.getCandles_1(product, granularity, start, end));
  }
}
