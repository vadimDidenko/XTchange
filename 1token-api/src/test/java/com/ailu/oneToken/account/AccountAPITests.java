...
public class AccountAPITests extends BaseTests {
  ...
  public void getAccountListTest() {
    try {
      accountService.getSubAccountList(AccountEnum.OKEFLDD).stream()
  ...
  public void getPurgeAccountListTest() {
    try {
      accountService.getPurgeSubAccountList(AccountEnum.OKEFLDD).stream()
          .forEach(o -> System.out.println(o));
    } catch (Exception e) {
      e.printStackTrace();
      Assert.assertTrue(false);
  ...
  public void addAsset2SubAccountTest() {
    try {
      boolean success =
          accountService.addAsset2SubAccount(
              AccountEnum.BICOIN.getExchange(), "bicoin_0", "eth", BigDecimal.valueOf(0.01));
