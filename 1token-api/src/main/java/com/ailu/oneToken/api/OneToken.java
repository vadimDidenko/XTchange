...
/** NOTE: @Version 1.0 @Since JDK1.8 @Author mr.wang @Company Henan ailu @Date 2018/12/13 17:09 */
public interface OneToken {

  String baseUrl = "https://1token.trade/api/v1/";

  @GET("quote/single-tick/{exchange}/{contract}")
  Call<String> getTickByExchange(
      @Path("exchange") String exchange, @Path("contract") String contract);
  ...
  Call<String> getOrders(
      @Path("exchange") String exchange, @Path("accountName") String accountName);

  @GET("trade/{exchange}/{accountName}/dealt-order/orders?")
  Call<String> getOrdersHistory(
  ...
  Call<String> getOrders(
      ...
      @Path("accountName") String accountName,
      @Query("state") String state,
      @Query("client_oid") String client_oid,
      @Query("exchange_oid") String exchange_oid,
      @Query("contract") String contract);
  ...
  /**
   * 下单 { "contract": "string", "bs": "b", "price": 0, "amount": 0, "client_oid": "string",
   * "options": { "additionalProp1": "string", "additionalProp2": "string", "additionalProp3":
   * "string" } }
   *
   * @param exchange
   * @param accountName
  ...
  Call<String> addAsset2SubAccount(
      @Path("exchange") String exchange,
  ...
  Call<String> getSubAccountList(
      @Query("category") String category, @Query("parent") String parent);
