...
public class OneTokeListener {
  ...
  public interface OneTokenApi {
    /**
     * 账户信息订阅接口 eg : {account_symbol} 形式大致为：huobip/ot-user1
     * wss://1token.trade/api/v1/ws/trade/{account_symbol} 签名验证 打开websocket连接时需要通过HTTP headers做验证:
     *
     * <p>Api-Nonce: 一个递增的数字，0到2^53
     *
     * <p>Api-Key: 你的 onetoken API key
     *
     * <p>Api-Signature: 请求签名，计算方法hex(HMAC_SHA256(ot_secret, verb + path + nonce +
     * data))，这里的path为/ws/{account_name}， 例如/ws/ot-user1，ot-user1是account symbol中的账户名部分。
     *
     * <p>Api-Meta: OT Account name（可选）.
     */
     ...
     * // 订阅账户信息 {"uri":"sub-info"}
     *
     * <p>// 订阅成功响应 {"action": "sub-info", "status": "ok"}
     *
     * <p>// 推送信息格式 {"action": "info","status": "ok", "data": {acc-info}}
     *
     * <p>// 订阅订单变化 {"uri":"sub-order"}
     *
     * <p>// 订阅成功响应 {"action": "sub-order", "status": "ok"}
     *
     * <p>// 推送信息格式 {"action": "order","status": "ok", "data": [order]}
     *
     * <p>//错误信息格式 {"action": "sub-info", "status": "failed", "message": "error message"}
     */
    String SEND_MSG =
    ...
    default WSListener wsByUrl(String url) {
      return new WSListener().url(url).ping("{'url':'ping'}", 5).parse(OneTokeListener::parse);
    }
  ...
  public static OneTokenApi oneTokenApi =
  ...
  public static void main(String[] args) {
    try {
      ...
      Request req =
          new Request.Builder() // .headers(OneTokenApi.headers())
              .url(OneTokenApi.TICK_WS_URL)
              .build();
      ws.url(OneTokenApi.TICK_WS_URL).message(msg -> System.out.println(msg)).start(req);
    } catch (Throwable e) {
      e.printStackTrace();
    }
  }
