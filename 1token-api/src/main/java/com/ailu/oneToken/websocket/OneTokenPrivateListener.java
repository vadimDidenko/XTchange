...
public class OneTokenPrivateListener {
  ...
  private static WSListener.Info parse(String text) {
    return Optional.of(text)
        .filter(str -> !"\"uri\": \"pong\"".contains(text))
        .map(OneTokenPrivateListener::apply)
        .orElse(null);
  }
  ...
  public static OneTokenPrivateListener.OneTokenApi oneTokenApi =
  ...
  public static void main(String[] args) {
    try {
      ...
      Request req =
          new Request.Builder()
              .headers(OneTokenApi.headers(OneTokenApi.APP_KEY, OneTokenApi.APP_SECRET))
              .url(OneTokenApi.WALLET_URL)
              .build();
