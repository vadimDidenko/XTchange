...
public class AccountServiceImpl implements AccountService, BaseService {
  ...
  public Account getAccount(String exchange, String accountName) {
    return strToModel(
        this.oneTokenClient.client.executeSync(
            this.oneTokenClient.api.getInfo(exchange, accountName)),
        Account.class);
  }
   ...
   * @param parent parent account, must be with format exchange/account e.g. okef/6b
   * @param account sub account which to be generate, with a letter head and it's length between 3
   *     and 20
   * @throws Exception when generate error occurs.
   */
  @Override
  public boolean genNewSubAccount(String exchange, String parent, String account) throws Exception {
  ...
  public List<String> getSubAccountList(AccountEnum accountEnum) throws Exception {
    String json =
        exec(
            oneTokenClient.api.getSubAccountList(
                accountEnum.getCategory(), accountEnum.getOTParamAccount()));
  ...
  /**
   * Get purge sub account list with specified parent account. Sub account looks like "bincoin_0"
   *
   * @param accountEnum
   * @throws Exception
   */
   ...
   */
  @Override
  public boolean addAsset2SubAccount(
      String exchange, String subAccount, String currency, BigDecimal amount) throws Exception {
  ...
  public String genNewTxAddress(String currency) throws Exception {
    ...
    if (ONE_TOKEN_SUCCESS.equals(code) && addr instanceof String) return (String) addr;
    return null;
  }
