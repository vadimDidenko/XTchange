...
/** NOTE: @Version 1.0 @Since JDK1.8 @Author mr.wang @Company Henan ailu @Date 2018/12/15 15:14 */
public class TradeServiceImpl implements TradeService, BaseService {
  ...
  /**
   * 获取订单列表 必填参数 exchange 交易所标识 eg:okex合约 okef,okex现货 okex
   *
   * @param exchange
   * @param accountName
   * @return
  ...
  public List<Order> getOrders(String exchange, String accountName) {
    return strToModels(
        this.client.executeSync(this.api.getOrders(exchange, accountName)), Order.class);
  }
   ...
   * @param exchange
   * @param accountName
   * @param state 订单状态 Available values : waiting, pending, withdrawing, withdrawn, dealt,
   *     part-deal-pending, part-deal-withdrawn, part-deal-withdrawing, error-order, active, end
  ...
  public List<Order> getOrders(
    ...
    return strToModels(
        this.client.executeSync(
            this.api.getOrders(exchange, accountName, state, client_oid, exchange_oid, contract)),
        Order.class);
  }

  /**
   * 获取成交历史 必填参数 exchange 交易所标识 eg:okex合约 okef,okex现货 okex
   *
   * @param exchange
   * @param accountName
  ...
  public List<Trans> getTransHistory(String exchange, String accountName, Long start, Long end) {
    return strToModels(
        this.client.executeSync(this.api.getTransHistory(exchange, accountName, start, end)),
        Trans.class);
  }
  ...
  public List<Trans> getTrans(String exchange, String accountName) {
    return strToModels(
        this.client.executeSync(this.api.getTrans(exchange, accountName)), Trans.class);
  }

  /**
   * 根据请求参数获取成交记录列表 必填参数 exchange 交易所标识 eg:okex合约 okef,okex现货 okex
   *
   * @param exchange
   * @param accountName
  ...
  public List<Trans> getTrans(String exchange, String accountName, String count, String contract) {
    return strToModels(
        this.client.executeSync(this.api.getTrans(exchange, accountName, count, contract)),
        Trans.class);
  }
  ...
  public List<Order> getOrdersHistory(
      String exchange, String accountName, Long start, Long end, String state) {
    return strToModels(
        this.client.executeSync(
            this.api.getOrdersHistory(exchange, accountName, start, end, state)),
        Order.class);
  }

  /**
   * 创建订单 必填参数 exchange 交易所标识 eg:okex合约 okef,okex现货 okex
   *
   * @param exchange
   * @param accountName
   * @param jsonObject
  ...
  public JSONObject placeOrder(String exchange, String accountName, JSONObject jsonObject) {
    return strToModel(
        this.client.executeSync(this.api.placeOrder(exchange, accountName, jsonObject)),
        JSONObject.class);
  }
   ...
   */
  @Override
  public JSONArray cancelOrder(
      String exchange, String accountName, String clientOid, String exchangeOid) {
    return strToModel(
        this.client.executeSync(
            this.api.cancelOrder(exchange, accountName, clientOid, exchangeOid)),
        JSONArray.class);
  }
  ...
  public JSONObject cancelAllOrder(String exchange, String accountName, String contract) {
    return strToModel(
        this.client.executeSync(this.api.cancelAllOrder(exchange, accountName, contract)),
        JSONObject.class);
  }
